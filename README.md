...
4. 屏幕底部会出现 **「🎤 麦克风使用中」** 提示条，并显示实时音量
5. 说话完毕后松开快捷键，识别结果自动插入光标处
6. 若音量过小，会弹出提示并发送系统通知
7. **左键点击**托盘图标，或右键选择 **历史剪贴板**，查看语音转换历史（支持搜索）
8. 右键托盘图标 → **选择麦克风**，切换录音输入设备
9. 右键托盘图标 → **修改热键**，可录制自定义 Push-to-Talk 快捷键（立即生效）
10. 右键托盘图标 → **文字输出**，可切换 **简体中文** / **繁體中文**
...
| `show_recording_indicator` | 录音时显示麦克风使用提示 | `true` |
| `low_volume_rms_threshold` | 音量过小判定阈值（0–1） | `0.015` |
